...
export async function fetchBudgetVsActual(startYear: number, years: number, initialCash: number) {
  const params = new URLSearchParams({ start_year: String(startYear), years: String(years), initial_cash: String(initialCash) })
  const res = await fetch(`${API_URL}/budget-vs-actual?${params.toString()}`)
  if (!res.ok) throw new Error('Erreur lors du calcul budget vs réalisé')
  return res.json()
}
